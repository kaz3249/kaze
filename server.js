...
async function createNowPaymentsCheckout({ orderId, productId, priceUsd }) {
  ...
  const headers = { 
    'x-api-key': process.env.NOWPAYMENTS_API_KEY.trim(), // .trim() removes extra spaces
  ...
  const body = { 
    ...
    price_currency: 'usd', 
    order_id: orderId, 
    order_description: `Product: ${productId}`, 
...
async function createOrderCore({ productId }) {
  ...
  try { 
    checkout = await createNowPaymentsCheckout({ orderId, productId: product.id, priceUsd: product.price_usd }); 
  } catch (err) { 
    await sendTelegram(`⚠️ Payment failed for ${product.id}: ${err.message}`); 
    throw err; 
  }
  
  const status = normalizeStatus(checkout.payment_status || 'pending');
  db.prepare(`INSERT INTO orders (id, product_id, status, price_usd, checkout_url, nowpayments_payment_id) VALUES (?, ?, ?, ?, ?, ?)`)
    .run(orderId, product.id, status, product.price_usd, checkout.checkout_url, checkout.payment_id);
...
// MAIN STORE PAGE
app.get('/', (_req, res) => {
  const allProducts = db.prepare('SELECT id, name, price_usd, image_url FROM products ORDER BY created_at DESC').all();
    ...
    productPages.forEach((pageProducts, index) => {
      ...
      const cardsHTML = pageProducts.map((p) => `
        <div class="product-card">
          ${p.image_url ? `<img src="${escapeHtml(p.image_url)}" class="product-img" onerror="this.style.display='none'">` : ''}
          ...
          <form method="POST" action="/buy"><input type="hidden" name="product_id" value="${escapeHtml(p.id)}"><button type="submit" class="buy-btn">Purchase</button></form>
        </div>`).join('');
      productsHTML += `<div class="product-page-block">${pageLabel}<div class="products-grid">${cardsHTML}</div></div>`;
    });
  }

  res.send(`<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>KAZE Studio</title><link href="https://fonts.googleapis.com/css2?family=Playfair+Display:ital,wght@0,400;1,400&family=Inter:wght@300;400;500;600&display=swap" rel="stylesheet"><style>
    ...
    .nav-links { display: flex; gap: 40px; } .nav-links a { color: #888; text-decoration: none; font-size: 0.85em; letter-spacing: 2px; } .nav-links a:hover { color: #fff; }
    ...
    .buy-btn { width: 100%; background: #7c6ff7; color: white; border: none; padding: 14px; border-radius: 8px; font-weight: 500; cursor: pointer; } .buy-btn:hover { background: #6b5ce6; }
  ...
  <section id="home">
    ...
    <div class="trust-badge"><span class="trust-stars">★★★★★</span> Trusted by collectors in 40+ countries</div>
  </section>
